...
<a></a>태그가 하이퍼링크라는 것을 저번 시간에 배웠다. 따라서 <a href=?getFlag>[Get Flag]</a> 는 [Get Flag]라는 이름의 하이퍼링크를 누르면 Document.URL의 하위 ?getFlag로 넘어간다는 뜻이다. Access Denied 경고창을 끄지 않고도 주소창을 건들 수 있어서 그 주소 뒤에 '?getFlag'를 붙여서 다시 연결하자.
